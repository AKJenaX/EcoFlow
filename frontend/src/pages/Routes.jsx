  ...
  return (
    <section className="fade-in space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        <button className="flex items-center gap-2 rounded-lg bg-[#84cc16] px-4 py-2 font-medium text-[#1a3a2a] shadow-md transition-all hover:shadow-lg">
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-slate-200 bg-slate-50">
                <th className="px-6 py-3 text-left text-xs font-semibold text-slate-600">Route ID</th>
                ...
                <th className="px-6 py-3 text-left text-xs font-semibold text-slate-600">Vehicle</th>
                <th className="px-6 py-3 text-left text-xs font-semibold text-slate-600">Stops</th>
                <th className="px-6 py-3 text-left text-xs font-semibold text-slate-600">Status</th>
                <th className="px-6 py-3 text-left text-xs font-semibold text-slate-600">Start Time</th>
              ...
              {routesData.map((route) => (
                <React.Fragment key={route.routeId}>
                  <tr className="hover:bg-slate-50 transition-colors">
                    <td className="px-6 py-4 font-medium text-slate-900">{route.routeId}</td>
                            ...
                            {route.binStops.map((stop) => (
                              ...
                              >
                                <div className="flex items-center justify-between mb-2">
                                  <span className="font-semibold text-[#1a3a2a]">{stop.binId}</span>
                                  <span className="text-xs font-medium text-slate-600">{stop.fillLevel}%</span>
                                </div>
      ...
      <div className="rounded-lg border border-slate-200 bg-slate-50 px-6 py-4">
        <p className="text-sm text-slate-600">
          Showing <span className="font-semibold">{routesData.length}</span> collection routes
        </p>
      </div>
    </section>
  );
